...
The atmospheric chemistry in AQ models is described by the chemical mechanism used by the AQ model; the chemical mechanism includes rate coefficients, reaction pathways with the corresponding branching ratios, photolysis rates and reaction products which are required to solve the concentrations of each chemical species within the system using Equation \eqref{e:conc}.
The level of detail included in the chemical mechanism determines the amount of computing resources required for the model simulations, thus, the chemical mechanism in a 3-D model will typically be less detailed than the chemical mechanism used by a box model.

As different modelling scopes and models determine the level of chemical detail of the chemical mechanism, many different chemical mechanisms have been developed by the AQ modelling community.
Chemical mechanisms can range from self-generating mechanisms, such as \citet{Aumont:2005}, which attempt to fully describe the chemistry occuring within the boundary layer, to %%here
write out the chemical reactions occuring at each time step using a 'generator' program to 
Self-generating (or explicit) chemical mechanisms are extremely detailed and are an attempt to fully describe the thousands of reactions taking place within the boundary layer.
